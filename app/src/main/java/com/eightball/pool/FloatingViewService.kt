...
class FloatingViewService : Service() {
    private var wm: WindowManager? = null; private var fv: View? = null
    ...
    override fun onCreate() {
        ...
        val layout = LinearLayout(this).apply {
            ...
            addView(ImageView(this@FloatingViewService).apply { setImageResource(android.R.drawable.ic_menu_more); layoutParams = LinearLayout.LayoutParams(80,80) })
        }
        var ix=0; var iy=0; var itx=0f; var ity=0f
        layout.setOnTouchListener { _, e -> when(e.action) {
            MotionEvent.ACTION_DOWN -> { ix=params.x; iy=params.y; itx=e.rawX; ity=e.rawY; true }
            MotionEvent.ACTION_MOVE -> { params.x=ix+(e.rawX-itx).toInt(); params.y=iy+(e.rawY-ity).toInt(); wm?.updateViewLayout(layout,params); true }
            MotionEvent.ACTION_UP -> { if(Math.abs(e.rawX-itx)<5&&Math.abs(e.rawY-ity)<5) try { Runtime.getRuntime().exec(arrayOf("sh","/storage/emulated/0/patoteam/rish","/storage/emulated/0/patoteam/pato0.sh")) } catch(ex:Exception){}; true }
            else -> false
        }}
        fv=layout; wm?.addView(layout,params)
    }
    override fun onDestroy() { super.onDestroy(); fv?.let { wm?.removeView(it) } }
}
